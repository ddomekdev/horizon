...
if __name__ == "__main__":
    ...
    for year in range(max_allowed_date.year, today_date.year + 1):
        for month in range(1, 13):
            daily_files = conn.list_data_by_category(
                asset_class="stocks", data_type="day_aggs", year=year, month=month
            )
    ...
    current_files = os.listdir(os.getenv("EQUITY_DAILY_DATA_PATH"))
    ...
    for file in tqdm(all_files, desc="downloading files"):
        s3_key = file["key"]
        base_save_path = os.getenv("EQUITY_DAILY_DATA_PATH")
