...
# -*- coding: utf-8 -*-
"""
Polygon.io Trades ETL Pipeline
--------------------------------------
This script implements an ETL pipeline to download all available trades
...
if __name__ == "__main__":
    ...
    # override because trade data is huge
    max_allowed_date = today_date - datetime.timedelta(days=30)
    ...
    for year in range(max_allowed_date.year, today_date.year + 1):
        for month in range(1, 13):
            trade_files = conn.list_data_by_category(
                asset_class="stocks", data_type="trades", year=year, month=month
            )
            all_files += trade_files
    ...
    current_files = os.listdir(os.getenv("EQUITY_TRADES_DATA_PATH"))
    ...
    for file in tqdm(all_files, desc="downloading files"):
        s3_key = file["key"]
        base_save_path = os.getenv("EQUITY_TRADES_DATA_PATH")
    ...
    print("Successfully downloaded all trades data")
